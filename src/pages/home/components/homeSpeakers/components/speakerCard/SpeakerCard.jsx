...
const SpeakerCard = ({ name: name, speciality: speciality, review: review, image: image }) => {
  ...
  return (
    <div className="w-[90%] md:w-[60%] laptop:w-[80%] h-[550px] mx-auto mb-5 flex flex-col items-center justify-top bg-Mint-green border-solid border-4 border-White rounded-xl">
      <div
      ...
      <div className="w-full mb-5 p-5 flex flex-col justify-center items-center rounded-xl md:px-2 md:py-5">
        <h3 className="text-xl p-2 font-montserrat font-Bold text-Navy-blue md:text-2xl">{name}</h3>
        <h5 className="text-base font-montserrat font-Semibold mb-3 text-Navy-blue desktop: md:text-xl">{speciality}</h5>
        ...
        {review.map((item, index) => (
          <p key={index} className="font-montserratAlternate font-Regular text-sm text-center leading-relaxed md:text-xl text-Navy-blue">
            {item}
          </p>
        ))}
